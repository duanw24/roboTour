...
public class Graph {
    private List<Pair<Point, Direction>> walls = new ArrayList<Pair<Point, Direction>>();
    private int mx,my;
    ...
    public boolean nodeRaycheck(Point2D a, Point2D b) {
        ...
        for(Pair<Point, Direction> wall : walls) {
            Point p = new Point(wall.getKey().x*25,wall.getKey().y*25);
            Direction d = wall.getValue();
            if(d==Direction.NORTH) {
                if(line.intersectsLine(p.x,p.y+1,p.x+1,p.y+1)) {//System.out.println("NORTH");
                    return false;}
            }
            if(d==Direction.SOUTH) {
                if(line.intersectsLine(p.x,p.y,p.x+1,p.y)){//System.out.println("SOUTH");
                    return false;}
            }
            if(d==Direction.EAST) {
                if(line.intersectsLine(p.x+1,p.y+1,p.x+1,p.y)){//System.out.println("EAST");
                    return false;}
            }
            if(d==Direction.WEST) {
                if(line.intersectsLine(p.x,p.y,p.x,p.y+1)){//System.out.println("WEST");
                    return false;}
            }
        }
        return true;
